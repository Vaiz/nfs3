...
use super::iterator::MirrorFs2Iterator;
...
impl MirrorFs2 {
    ...
    /// Load metadata from filesystem for a given file ID
    async fn load_metadata(&self, id: fileid3) -> Result<fattr3, nfsstat3> {
...
impl NfsReadFileSystem for MirrorFs2 {
    ...
    async fn getattr(&self, id: &Self::Handle) -> Result<fattr3, nfsstat3> {
        ...
        // Load metadata directly from filesystem
    ...
    async fn readdir(
        ...
        let iter = MirrorFs2Iterator::new(fsmap, dirid, start_after).await?;
        Ok(iter)
    }
...
impl NfsFileSystem for MirrorFs2 {
    async fn setattr(&self, id: &Self::Handle, setattr: sattr3) -> Result<fattr3, nfsstat3> {
        ...
        // Load updated metadata directly from filesystem
        self.load_metadata(id).await
    }
